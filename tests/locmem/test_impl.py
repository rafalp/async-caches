...
@pytest.mark.asyncio
async def test_deleting_many_undefined_key_has_no_errors(cache):
    await cache.set("test", "Ok!")
...
@pytest.mark.asyncio
async def test_set_key_can_be_increased_by_a_value(cache):
    await cache.set("test", 10)
...
@pytest.mark.asyncio
async def test_set_key_can_be_decreased_by_a_value(cache):
    await cache.set("test", 10)
